...
>
> **Amended 2026-08-10** (logback flavor): O2 is no longer log4j2-only. `RenderOptions.WithLogs bool` is replaced by a `LogFlavor` (`LogNone` / `LogLog4j2` / `LogLogback`) and the single `_logs/` template marker by one per flavor (`_logs-log4j2/`, `_logs-logback/`) — both flavors render to the same `__PKG__/SuluLogAppender.java`, so a shared marker would collide. The **rendered class name and `drainCurrentThread()` API are identical across flavors**, so the `{{if .WithLogs}}` flush blocks in `SuluLabelListener`/`SuluAllureExtension` are unchanged and flavor-agnostic (the template data struct still exposes `WithLogs bool`). The logback appender is an `AppenderBase<ILoggingEvent>` with a `PatternLayoutEncoder` (concrete-typed setter so a class-less `<encoder>` element resolves) and the same `ThreadLocal<StringBuilder>` buffer; §5.3's plugin-discovery problem has no logback counterpart — logback resolves appenders by class name, so `LogbackSetupSteps` prints an `<appender class="…">` + `<appender-ref>` snippet with **no `packages=` equivalent**. The no-flavor-detected hint is broadened rather than introduced: `Log4j2HintSteps` already fired for a Java project without log4j2, but it named only log4j2 — i.e. it told a logback user to switch logging frameworks. `LogHintSteps` now names both, and calls out that a transitively-pulled logback (e.g. `spring-boot-starter-test`) is invisible to a build-file grep. §5.6's contract is unchanged and now locked by `TestRenderLogAttachmentContractIsLocked` across both flavors × both frameworks. Companion FE empty state: [ellyZz/sulu#284](https://github.com/ellyZz/sulu/pull/284).
>
